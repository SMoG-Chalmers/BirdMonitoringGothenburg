...
This document provides URI identifiers, definitions, and examples for all the attributes (column headers) used in the comma-delimited text files included in the data package (gothenburg_bird_data_package-v2.0) available to download at <https://doi.org/10.5281/zenodo.14629008>.
...
**Definition**:\
Indicates whether a bird detection is isolated within a 9-second window. A detection is considered isolated if it is not preceded or followed by another detection of the same species within 9 seconds.
...
-   **0.5 \< occurrenceAccuracy \< 1.00**: For non-expert-validated records (`expertValidated == "No"`), this value shows the model-predicted probability of occurrence, based on the trained Random Forest classifier.
